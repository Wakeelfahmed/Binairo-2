...
void GameBoard::init()
{
    ...
    for(unsigned int i = 0; i < SIZE; ++i)
    {
        std::vector<QPushButton*> buttonRow;  // Create a row vector for buttons
        for(unsigned int j = 0; j < SIZE; ++j)
        {
            QPushButton* button = new QPushButton;  // Create a new QPushButton
            buttonRow.push_back(button);  // Add the pointer to the button in the row vector
        }
        buttons.push_back(buttonRow);  // Add the row vector to the buttons vector
    }
}
...
int GameBoard::get_fillsymbol() const { return fillsymbol; }

void GameBoard::set_fillsymbol(int symbol){ fillsymbol = symbol; }
